...
- **下書きが参照する画像は GC しない**（区分 E の「参照」に `inbox_items.draft` の `tracks[].picture` を足す。
  ...
  配置は failed（選び直す）
- **途中で落ちた件の再承認では、登録前の孤児に限って宛先の自分の成果物を今回の内容で置き換える**
  （`place_one_refreshing`）。任意のタグや画像の補正はパスを変えないので同じ宛先に当たり、同じ音声だからと
  そのまま再利用すると今回の補正が黙って捨てられる（codex 指摘）。置き換えは tmp + rename で、確かめた FD と
  置き換える直前の宛先が同じ実体（dev / inode / size / mtime / ctime）でなければ Conflict。**宛先に active な
  行がある（登録済み）ファイルは置き換えない**: ファイルが正で、外部（foobar2000 等）の変更を履歴なしに上書き
  してはならない（codex 指摘）。その場合の今回の補正は書かず WARN を出す（ライブラリの編集で直す）
