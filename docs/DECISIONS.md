...
（Inbox のファイルの `PICTURE` を返す `GET /api/inbox/:id/artwork/:file` を足す）。P4-4。

**追記（2026-09-20、P4-4 の設計）**:

- 提案の `artist` は ARTIST の全値を **`"; "`** で結合する（foobar2000 等の多値の慣習。Library の一覧が
  `artist_display` に使う `", "` とは区別し、1 値の中の読点と紛れにくい）。配置の「未編集」判定は先頭値との一致
  から**結合文字列との一致**に変える（画面に見えているものがそのまま書かれる、が要点）。編集した値は
  `;` で分割せず 1 値で書く（分割規則を持ち込むと、`;` を含む 1 値のアーティスト名を表せない）。旧規則
  （先頭値との一致）は落とす。承認済みで未配置の件は実機に無いので互換は取らない
- 画像の口は **`GET /api/inbox/:id/artwork/:hash`**（TASKS の `:file` から変更）。ハッシュアドレスなら
  ...
  他の件のファイルを開かせないため。サムネイルは作らない（承認画面だけで使い、件は配置で消える。
  原寸を `<img>` で縮小して十分）
...
- 受け入れ: `tests/inbox_api.rs`（提案の結合、artwork の 200 / 304 / 404 / 401）、`tests/inbox_job.rs`（未編集で
  多値を保つ・編集で 1 値）、`web/src/lib/inbox.test.ts`（`pictureOf` / `itemCover` / `artistNote`）
...
ミラーのスクレイピング依存）。Cover Art Archive（MB で当たる盤のジャケットは取れるが、要望が無い。
アートワークは埋め込み / 同梱 / 手動で足りている。D-60）。AcoustID（配信音源を MB に結びつける穴を
