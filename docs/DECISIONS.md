...
- **下書きが参照する画像は GC しない**（区分 E の「参照」に `inbox_items.draft` の `tracks[].picture` を足す。
  ...
  配置は failed（選び直す）
- **途中で落ちた件の再承認では、宛先の自分の成果物を今回の内容で置き換える**（`place_one_refreshing`）。
  任意のタグや画像の補正はパスを変えないので同じ宛先に当たり、同じ音声だからとそのまま再利用すると
  今回の補正が黙って捨てられる（codex 指摘）。置き換えは tmp + rename で、登録は置き換えた後の inode を指す
