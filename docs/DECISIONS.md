...
- **アップサンプリングはカットオフ周波数だけで判定しない。** `cutoff_hz ≤ 24 kHz` に加えて、
...
- **カットオフの探索は 1/3 オクターブで平滑化した系列、崖は平滑化前の平均パワーで測る。**
  24 kHz での 1/3 オクターブ幅は約 5.5 kHz あり、平滑化した系列で前後 1 kHz を比べると崖を
  自分でぼかして 30 dB に届かなくなる。探索と計測で系列を分ける。境界（完全ゼロの帯域、上端まで
  信号がある本物、Nyquist 直下のカットオフ、全無音）は SPEC §7.10 で有限値か NULL に定めてある
