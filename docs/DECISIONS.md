...
- 下書きのトラックに **`keep_artists`**（ファイルの多値 ARTIST をそのまま保つ）を持たせ、意図を明示する。
  ...
  Library の一覧が `artist_display` に使う `", "` とは区別）。配置は true なら ARTIST に触れず、false なら
  `artist` の 1 値で置き換える（`;` で分割しない。分割規則を持ち込むと `;` を含む 1 値を表せない）。
  画面は元の値を境界の分かるチップで見せ、チェックを外すと 1 値の入力になる
...
- 代表画像: `inbox_files.tags` の `PICTURE` は画像種別を持たないので、走査が Library の `pick_embedded`
  ...
  代表の最頻（同数なら先に現れたもの）。Library に入る album の代表画像（同梱カバー → 最初のトラックの
  埋め込み）とは同じ規則で選んだ埋め込み画像になる
