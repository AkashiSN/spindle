...
**未決**: Safari 向けの AAC 変換（`transcode=aac`。ffmpeg の aac エンコーダで足せる）。ただし
Safari 18.4（2025-03。iOS / iPadOS / macOS）以降は Ogg Opus をネイティブ再生でき、FLAC / ALAC は
もともと再生できるので、今の Safari では変換なしで全曲再生できる見込み。実機で確認して閉じる。

**閉じた未決**（2026-09-18）: ハイレゾのサンプルレート変換。可逆は既定で Derived の Opus（48 kHz）を
