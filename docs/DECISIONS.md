...
- **ソートはホワイトリスト**（`album`（既定。albumartist, album_id, disc_no, track_no）/ `title` /
  ...
  NULL で不定になるので、
  ソート式は `coalesce(col, '' | 0 | -1)` に畳み、同じ式の**式索引**を `0001_init.sql`（旧 `0002_tracks_sort_indexes.sql`。D-88 で統合）
...
**追記（2026-09-25。ファイル名の書式）**: `[layout]` の既定（`config.example.toml` / SPEC）を `{track:02}. {title}`
（番号の後にピリオド）にする。multi_disc は `{disc}-{track:02}. {title}`。理由: 旧パイプラインから移した Library の
約 7,500 曲が `01. タイトル` で、購読の番号揃えや Inbox の配置が付ける `165 タイトル`（ピリオド無し）と同じ
ディレクトリで混ざった（リハーサルの明透・理芽）。既存の多数派に合わせ、混ざった分はリネームで揃える。購読の
番号揃えの改名判定（ファイル名の先頭の番号）は数字の続く部分だけを読むので両方の形で同じ番号になる
（`leading_number` の単体試験）。テストが自前で与える layout はそのまま（書式に依存しない）。

**追記（2026-09-25。edition の供給源と、片方だけが edition を持つ衝突）**: `{album} ({edition})` の降格は
`albums.edition` を使うが、それまでスキャナがこの列に値を入れる処理が無く、全 album で NULL（edition の段は一度も
効いていなかった）。実機の IM@S CM Solo の 2 枚（通常版 4 曲と、`Hi-Res/` サブフォルダの 1 曲。ALBUM / ALBUMARTIST /
DATE が同じ）がリネームで「降格に必要な値が無い: edition」になって見つかった。
...
- **最後の段で edition を持たないリリースは元の `{album}` のまま**（ユーザの判断）。通常版 + Hi-Res 版は通常版が
  ...
  年の段は従来どおり（年が違えばそこで分かれる）。既にいるリリースが 1 つだけでそれと同じなら合流する規則も
  従来どおりで、宛先に通常版が既にいる場合は新しく来た方が年の段で分かれる（edition の段まで行かない）
- **却下**: 両方に EDITION を付けて規則を変えない案（通常版に「CD」等の値を付けるだけの手間が増え、既存の
  フォルダ名も変わる）
...
**追記（2026-09-25。保持期間の既定）**: `[gc].retention_days` の既定（`config.example.toml` / SPEC）を 30 → **7 日**
にする（ユーザの判断で保持を短くする）。対象は retention に従う GC の区分すべて（`missing_since` の行、Archive へ
退避した原本、Derived の孤児、D-90 の破棄待ちの Inbox の取り込み）。影響: 正規化で退避した原本を「退避ファイル」から
戻せる期間と、missing の行・破棄待ちを取り消せる猶予も 7 日になる。コードに既定値は無く（設定の必須キー）、
テストが自前で与える値はそのまま。1 日 1 回の定期実行はそのままで足りる
...
**追記（2026-09-25。P4-22。追記のディスク番号）**: 追記先の album の active な曲がどれも `disc_no` を持たず、下書きの
`disc_no` の最大が 1（人が 2 以上にした値は捨てずに書く）で CD の取り込みでないときは、配置で `DISCNUMBER` を**書かず、件のファイルにあれば消す**
（`import::inbox::omits_disc`。`destination.uses_disc` を応答にも載せ、画面は同じ規則で disc 列を空で見せる）。
理由: YouTube の件は提案が disc 1 固定で、旧パイプライン由来の disc なしの album（Library の 6,659 曲）に追記すると、
disc → track の並べ替えで追記した曲だけが末尾に回った（リハーサルの明透 165・理芽 233。バッチ 22 で手当て）。
ファイルが持っていた `DISCNUMBER` も消すのは、ファイルが正（DB は `disc_no` をタグから登録する）で、1 つでも残ると
同じ並びの問題が戻るため。下書きの `disc_no` は u32 のまま 1 で持つ（番号の衝突判定は既に NULL を 1 として扱う）。
宛先がディスク番号を使っている album・CD の件（複数枚の medium 位置）・新規の album は従来どおり書く。
...
## D-87 YouTube 画面と一括編集・操作タブにも番号付きの段の導線を入れる

**決定**（2026-09-24。ユーザ要望。モックで合意）:

- **YouTube 画面は「ダウンロード」と「購読」を切り替え、どちらも番号付きの段にする。** ダウンロードは
  ① URL → ② 取り込み方 → ③ ダウンロード → ④ Inbox で承認、購読は ① 再生リスト → ② アルバムとして登録 →
  ③ 登録して同期。④ は置いた件を並べて「Inbox で開く」だけにし、承認そのものは Inbox の画面で行う
  （承認画面を二か所に作らない）
- **① で取り込み済みかを先に見せる**（`POST /api/ytmusic/lookup`。DB だけ）。動画は正規形
  `https://www.youtube.com/watch?v=<id>` にしてファイルの `SOURCE_URL` と照合する（ダウンローダの判定と同じ）。
  `list=` 付きの動画 URL は yt-dlp と同じく再生リストとして扱う。YouTube 以外の http / https は照合せずに
  そのまま投入する（yt-dlp の対応サイトは広い）。照合は目安で、投入側の飛ばし（D-70）はそのまま残す
- **再生リストの中身は別の API で引く**（`POST /api/ytmusic/playlist`）。yt-dlp の列挙は数秒かかりネットワークに
  出るので、照合（毎入力）とは分けて URL ごとに 1 回だけ呼ぶ。読むだけでジョブにしない。同時に 2 本まで、
  90 秒で打ち切り、接続が切れたら yt-dlp も止める。新規が 0 の再生リストは投入しない（取りこぼしがあれば投入する）
- **③ ④ が追う「今回の投入」はサーバが返したジョブと、その展開の子に限る。** 再生リストの展開が投入する
  ytdl の payload に `parent_job_id` を入れる（ハンドラは読まない）。id の大小で「以降」を今回とみなすと、
  購読の同期や別の画面の投入、重複で返った古い id 以降の履歴まで混ざる（codex 指摘）
- **一括編集・操作タブは段を横に 4 列並べる。** パネルは表の上で高さが 260px 前後の横長なので、縦に積むと
  スクロールが要る。コンテナ幅が狭ければ 2 列 → 1 列
- **操作タブは操作を 1 つ選ぶ形にする。** これまでは全部の操作を縦に並べていたが、③ と ④ をその操作用にし、
  「巻き戻せる変更 / ジョブ / 読むだけ / すぐ反映」を印で示す。④ は押せない理由（プレビュー待ち・画像未選択・
  追加先未選択）を添える
- **操作タブで操作を切り替えたら、前の操作の結果・失敗・反映待ちの確認を消す。実行中は切り替えない。**
  反映待ちの「除外して適用」は保持中の操作をやり直すので、別の操作の段に出ると別の操作を実行してしまう
  （codex 指摘）
- **プレビューが古くなったことを見せる。** これまでは古いプレビューを黙って消していた。③ を「古い」にして
  枠を警告色にし、④ を押せなくする（`useBatchEdit.stale` / `useOperations.pathStale`）

**理由**: CD / Inbox で入れた導線（D-85、D-86）を他の取り込み・編集の画面にもそろえ、どこまで済んだか、
次に何をするかを同じ形で読めるようにする（ユーザ要望）。

**却下**: YouTube ④ で承認まで済ませる（Inbox の承認画面と重複する）。操作タブを全部並べたまま段だけ付ける
（操作ごとに ③ ④ の中身が違い、段が意味を持たない）。再生リストの列挙を照合 API に含める（入力のたびに
yt-dlp が走る）。

## D-88 マイグレーションを最初のリリースの前に 1 回だけ `0001_init.sql` へ畳む

**決定**（2026-09-24。ユーザ依頼。D-79 / TASKS P4-12 (8) の方針どおり）:

- `db/migrations/0001`〜`0024` を、最終形のスキーマを直接作る `0001_init.sql` 1 本に置き換える。列の並び・
  CHECK・既定値・索引・トリガ・ビュー・種データ（`export_profiles` の 3 行）は 24 本を流した結果と同じ
  （空 DB へ流した `sqlite_master` をコメントと空白を除いて比べ、全表の `table_xinfo` / `index_list` /
  `index_xinfo` / `foreign_key_list` と行を比べて一致を確かめた。違うのは `sqlite_sequence` に
  `playlist_subscriptions` の行（seq 0）が無いことだけで、次の id が 1 なのは同じ）
- 0006 の Rust 側の手当て（既存プレイリスト名の衝突の改名と `idx_playlists_name_key` の作成。`post_sql`）は
  消し、索引は SQL に直接書く。`foreign_keys=OFF` で流す版の一覧（`FOREIGN_KEYS_OFF`）は空にして仕組みは
  残す（以後 CHECK を広げるために表を作り直す版で使う）。各版のアップグレード試験は、それが守っていた
  最終スキーマの性質を空 DB に対する試験へ移した（`tests/migrations.rs`）
- 版 24 までの既存 DB を新しいバイナリで開くと `Newer`（DB の版 24 > バイナリの最新 1）で起動しない。
  `edge` は DB の互換を約束しない（D-79）ので許容し、リハーサル環境はリリース時の再移行で DB ごと作り直す
  （MIGRATION §5）。以後は CLAUDE.md の禁止事項どおり、既存ファイルは書き換えず連番を足す

**理由**: 公開イメージを誰かが pull して DB を作った後は既存ファイルを書き換えられないので、畳めるのは
最初の `vX.Y.Z` より前、DB を作り直すこの時だけ。24 本のうち 8 本は表の作り直し（CHECK を ALTER できない
ため）で、新しい DB を作るたびに同じ表を何度も作っては捨てていた。スキーマの正を 1 ファイルで読めるように
する。

**却下**: 畳まずに残す（空 DB の構築が作り直しの連鎖のままで、スキーマの全体を読むのに 24 本を追う必要がある）。
版 24 の既存 DB を新しい 0001 の版 1 へ読み替える手当てを入れる（互換を約束しない `edge` のためだけの
コードが残り続ける。リハーサル環境は作り直す）。
...
## D-89 ALAC のプロセス内デコードの範囲を本番の ffmpeg に揃える（MP4 の edit list と末尾の長さ 0 のサンプル）

**現行の規則は末尾の追記（2026-09-26）。** 以下の「旧決定」（宣言長で打ち切る）は追記で廃止した。

**旧決定（追記で廃止）**（2026-09-24。リハーサルの FLAC 正規化で見つかった不一致への対応）:
...
失敗した（照合が働き、元ファイルは無傷）。これらの MP4 は `stts` が `[(N, 4096), (1, 0)]` で、最後のサンプルの
長さを 0 と宣言している（`mdhd` の duration も N×4096）。ffmpeg は宣言どおりそのサンプルを捨てるが、
symphonia 0.6.1 の MP4 読みはパケットとして返し、デコーダは 1,043〜1,664 フレームを余分に出す。
共通部分の PCM はビット単位で一致しており（内容の違いではなく長さの違い）、余分な部分はほぼ無音
（1 本は全 0、1 本は最大でも −101 dBFS 程度）。宣言長で打ち切ると 2 本とも ffmpeg と同じ MD5 になる。

**理由**: ユーザは当初「ALAC のデコードを ffmpeg に揃える」を指示したが、正規化は元ファイルを symphonia、
生成物を ffmpeg で読んで突き合わせる（D-46「独立した 2 つのデコーダ」）ので、ALAC を ffmpeg にすると照合が
自己比較になって安全弁の意味を失う。原因が長さの宣言の解釈だけで、symphonia の PCM 自体は正しいので、
宣言長に従わせる方が変更も小さく照合も保てる。ユーザはこの案を選んだ。
...
**追記（2026-09-26。本番の再移行で見つかった打ち切りすぎ。ユーザ依頼）**: 本番の再移行の FLAC 正規化で、ALAC 46 本が
逆向き（symphonia の方が短い）の不一致で止まった。上の「ffmpeg は宣言どおり捨てる」は誤りで、ffmpeg は `stts` ではなく
edit list（`elst`）に従っていた。上の 7 本は `elst` の終端が宣言長ちょうど、46 本は `mvhd`（1/1000 秒）への切り上げで
3〜64 サンプル後ろにあり、長さ 0 のパケットの開始（= 宣言長）が終端より前なので ffmpeg はそれを丸ごとデコードしていた。
46 本の余分は 64〜4,096 フレームで、43 本は無音か −100 dBFS 以下、3 本は −53〜−83 dBFS のフェードの末尾（本物の音声）。
上の回帰試験のフィクスチャ（`stts` と `mdhd` だけを縮める）も終端が後ろの形で、ffmpeg は実際には捨てていなかった。

- **本番の ffmpeg（5.1）の規則**（実 ALAC と、実 ALAC から stream copy・`elst` の書き換えで作った形で確かめた）:
  先頭は media_time サンプルちょうど削る。終端は media_time + 区間の長さ（movie timescale → media timescale を
  四捨五入。端数 0.4 は切り捨て、0.5 は切り上げ）。終端以降で**始まる**パケットは捨て、終端をまたぐパケットは
  丸ごと残す（長さ 0 のパケットも同じ規則）。edit list が無ければ何も削らない。「空の編集 + 区間」の 2 項
  （出力側 `-ss` の stream copy が書く）は edit list を無視して全パケットを出す
- **ffmpeg の版で違う**: 新しい ffmpeg（開発機の N-125084、2026-06）は終端で端数を切り（長さ 0 のパケットだけは丸ごと）、
  「空の編集 + 区間」では末尾を空の編集の長さだけ削る。先頭の削り・終端 = 全パケットの終わりの形・長さ 0 の
  パケットの扱いは 5.1 と同じ
- **決定**: `FrameLimit` を「宣言長で打ち切る」から「本番の ffmpeg 5.1 の出力範囲を再現する」に変える。ALAC で、
  `elst` が 1 区間・media_time ≥ 0・等速・長さ > 0 のとき、先頭を media_time 削り、終端以降で始まるパケットを読まない
  （`media::mp4edit` で `moov` から読む。symphonia は edit list を読まないため）。`elst` が無い・単純な形でない・
  `mdhd` の timescale がサンプルレートと違うときは何も削らない（「空の編集 + 区間」は 5.1 と一致する）。
  edit list は symphonia が選んだトラック（`Track::id` = `tkhd` の track ID）のものだけを使う（symphonia は codec の
  分かる最初の音声トラックを選ぶので、先頭の音声トラックとは限らない）。壊れた・悪意のあるファイルで膨らまないよう、
  `moov` 64 MiB・トップレベルの箱 64 個・音声トラック 8 本・保持する `elst` の項 4 件を上限にし、子の箱は確保せずに辿る
  （codex のレビューの指摘）
- **理由**: 正規化の照合（D-46）は元ファイルを symphonia、生成物を本番の ffmpeg で読むので、両者の出力範囲が同じでないと
  必ず失敗する。46 本の余分を捨てる（FLAC 側も宣言長で切る）とフェードの末尾（最大 85 ms）を失い、ALAC のまま残すと
  正規化の対象から恒久的に外れる。ユーザは ffmpeg の実際のデコード長に合わせる案を選び、実在しない形（先頭の削り・
  終端がパケットの途中や手前）も実 ALAC で確かめながら再現するよう指示した
- **確かめたこと**: 実 ALAC 53 本（上の 7 本と 46 本）と、実 ALAC から作った 31 の形、長さ 0 のサンプルを持たない
  `elst` 付きの実 ALAC 9 本で、`decoded_pcm_md5` が本番コンテナの ffmpeg 5.1 で作った FLAC の STREAMINFO MD5 と一致
- **ffmpeg を上げるとき**（ベースイメージの Debian を上げると 7.x になる）: 終端をまたぐパケットと「空の編集 + 区間」の
  扱いが変わる。実在する形（edit list が無い・終端 = 全長・長さ 0 のパケット）は版で変わらないので既存の 46 本 + 7 本は
  影響を受けないが、変わる形のファイルは照合で止まる（音声は失わない）。そのときは `FrameLimit` を新しい版の規則に
  合わせ、`tests/fingerprint.rs` の 5.1 に固定した期待値を直す
- **既存 DB への影響**: 46 本は次の deep scan で `audio_md5` が変わり、`audio_version` が上がって RG と Derived が
  その 46 本だけやり直される。7 本の形と、edit list を持たない ALAC は値が変わらない
- **却下**: 正規化の照合で末尾の差を許す（本物の欠損・切り詰めも通してしまう）。ALAC を ffmpeg でデコードする
  （本文と同じく照合の独立性を失う）。新しい ffmpeg の規則に合わせる（本番は 5.1 で FLAC を作る）

## D-90 却下した Inbox の件は「削除」で破棄待ちにし、GC が retention 日後に消す
...
- **物理削除は GC だけ**（CLAUDE.md の禁止事項）。GC の区分 F: `rejected` で `discard_requested_at <= now -
  ...
  もの）・既知の同梱ファイル（`cd::riplog::is_companion_name`。配置が Library へ持っていくものと同じ）・サイドカー
  （確かめたときと同じもの）だけ。ディレクトリは空になったときだけ消す。サブディレクトリ（別の件）・symlink・
  知らないファイルには触らない。Inbox 直下の件（`rel_dir` が空）は音声だけ。猶予は Library の論理削除と同じ
  `retention_days`（別の設定を増やさない）
- **破棄待ちの間にファイルが変わったら消さない**（安全側）。走査が変化（足された・差し替えられた）を見たら破棄待ちを
  解き、`rejected` のまま `error` に理由を残す（`pending` には戻さない: 却下は人の判断で、戻すと上部バーの件数に
  出て承認を促してしまう。見直して要らなければもう一度削除する）。GC は**確かめ → 削除**の 2 段: 確かめの段で
  直下の音声を `inbox_files` と照合し（写した後に足された・変わったもの）、同梱ファイルとサイドカーは
  `inbox_files` に写らないので **ctime が「破棄の要求の秒 − 1」以降**なら要求の後に置かれた・変えられたものとみなす
  （境目は安全側に「後」。1 秒の余裕は、要求の時刻が実時刻の秒なのに ctime はカーネルの粗い時計で数 ms 遅れて付き、
  要求の直後に置いたファイルが要求の前の秒に見えることがあるため。試験で約 1/8 の頻度で見逃しを再現した。2026-09-25）。どれかが外れたら何も消さずに同じく解く。削除の段は 1 本ずつ、消す直前に確かめの
  段の stat（inode / size / mtime / ctime）と照合し、外れたらそこで止めて解く（それまでに消したのは確かめ済みの
  ものだけ）。NotFound 以外で消せなければそこで止めて**行と破棄待ちを残す**（成功扱いにしない。次の GC で続きを
  行う。消えたファイルは NotFound として飛ばす）。どちらの場合も行は消さない（codex レビューの指摘）
- `rejected` から出る遷移（下書きに戻す）は破棄待ちを解く（`transition` / `set_state` が `discard_requested_at` を
  NULL にする）
- GC の実行は件ごとに書き込みのトランザクションの中で「まだ rejected で期限が来ているか」を確かめ直し、ファイルを
  消してから行を消す。取り消しの API も同じ writer を通るので、確かめてから消すまでの間に取り消されることはない
- 画面: 一覧は従来どおり却下した件もバッジ付きで出す（SPEC の「一覧から外す」は実装と違っていたので直した）。
  却下した件に「削除」（確認ダイアログに消す時期）、破棄待ちには「削除待ち」のバッジと期限、「削除を取り消す」。
  期限は `GET /api/inbox` の `discard_retention_days` から

**理由**: 却下した件のファイルは Inbox に残り続け、人が SMB で消すしかなかった（リハーサルで VALIS の二重取り込みを
却下した後に観測）。却下 = 即削除にすると、下書きに戻して承認し直す道が消え、物理削除が GC 以外の経路に出る。
二段にして GC に任せれば、人が「要らない」と明示した件だけが猶予の後に消える。

**却下**: 却下と同時に削除する（戻せない）。確認ダイアログで即時に消す（物理削除が GC 以外に出る）。却下した件を
すべて GC の対象にする（「却下したが後で見直したい」件まで消える）。破棄待ちの専用の状態を足す（CHECK の作り直しが
要り、`rejected` と別に扱う箇所が増える）。

## D-91 CD の取り込みの表の画像は Cover Art Archive から一度だけ取り、承認画面の初期値に入れる

**決定**（2026-09-25。P4-22。ユーザの決定: 「CD の取り込みでリリースが決まっていて Cover Art Archive に front 画像が
あれば、承認画面の初期値に最初からその画像を入れる」）:

- **取るのは inbox ジョブの走査と配置の後に一度だけ。** 対象は承認前（`pending` / `failed`）で画像がまだ無く、試行が上限
  （2）未満の取り込み。サイドカーの `rip.metadata.release_id`（吸い出しで選んだリリース。MBID）があるものだけ
  取りに行く。承認画面を開くたび・`GET /api/inbox` のたびには外へ出ない（一覧の表示を外部の遅さに引きずらせない）。
  吸い出しの配置は inbox ジョブを投入するので、吸い出した直後の走査で取れる。この機能より前に Inbox に置いた取り込み
  （実機の嵐「Five」）も、次の走査で同じ条件に当たって取れる
...
- **配置を待たせない**: inbox ジョブ（並列 1・固定の dedup キー）の中で、**承認済みの配置を先に済ませてから**取る。
  1 回の実行で取りに行くのは 4 件（`CAA_PER_RUN`）・90 秒（`CAA_RUN_BUDGET`）まで（走行中の 1 件は
  `CoverArtClient` の全体 timeout 60 秒で終わる）。残りは周期の監視が「やることが残っている」
  （`needs_attention`。inbox ジョブに Cover Art Archive を渡したときだけ数える。渡していないのに数えると候補が
  減らず毎周回投入する）として次の周回で続ける。取り消しは通信の待ちの間も見る（`tokio::select!`）。別のジョブ
  種別に分けるのは `jobs.type` の CHECK の作り直しが要るので見送った
- **回数は外へ出る前に claim する**（ジョブの冪等）: `caa_tries` を読んだ値との CAS で 1 回分進めて永続化してから
  通信する（`claim_caa`）。結果の記録は回数を戻さない（`max`）。通信の途中で落ちても・取り消されても、その回は
  数えられ、再起動をまたいで上限を超えない
- **画面**: 開いている取り込みは、人がまだ触っていなければ、走査の後に変わった提案（表の画像が入った等）を取り込む
  （`refreshedDraft`。触っていれば上書きしない）。一覧のサムネイルは埋め込み画像が無ければ下書き・提案の画像
  （`/api/artwork/:hash`）を出す（`itemThumbUrl`）
- **再試行**: 画像の無い盤（404）と、リリースの無い取り込み（候補を選ばずに吸い出した・CD でない）は 1 回で上限に
  する。上流の失敗（5xx・通信・画像でない）と置けなかったときは、claim で進めた回数のまま次の走査でもう 1 回だけ試す。
  どれも取り込みは失敗させず、画像なしのまま（理由はログ）
- **提案**: `caa_picture` があれば全曲の `picture` の初期値にする。保存した下書きがあればそちらが勝つ（`merge_saved`。
  人が外した・差し替えたものを上書きしない）。配置は D-86 どおり `picture` のある曲の埋め込み画像をその 1 枚にする
- **画面**: 全曲がその画像のままなら「Cover Art Archive の表の画像（吸い出したリリースから自動で取った）」と出し、
  戻すボタンは「画像を外す」（`resetPictures`。CD のファイルには画像が無いので外すと画像なし）。手動の
  「Cover Art Archive から取る」は残す（リリースを引き直した後など）
- **GC**: `caa_picture` の画像は区分 E の参照とみなす（提案の初期値として配置まで要る）。取り込みの行が消えれば
  参照も外れる（配置した後は `tracks.artwork_id` が参照する）

**理由**: CD 画面では候補のジャケットが見えていた（D-82）のに、吸い出したファイルには画像が無く、承認画面で
「Cover Art Archive から取る」（D-86）を押さないと入らなかった。見えていたものが Inbox で消えたように見え、押し忘れる
と画像の無いまま配置される。候補を選んだ時点で盤は決まっているので、既定で入れて外せる形にする方が分かりやすい。

**却下**: 吸い出しの時点でファイルに埋め込む（D-67 の吸い出しは FLAC を作って Inbox に置くまでで、取り込みの内容は
...
## D-93 表の画像だけを同じリリースグループの別の版から取れるようにする

**決定**（2026-09-25。P4-23。ユーザの要望: 「MusicBrainz の候補に出ている別のリリース（BD 版や初回限定版）には
カバー画像があるが、取り込みたい通常盤には登録されていないとき、メタデータは通常盤のまま、カバー画像だけ別の
リリースにしたい」。自動の補完と手動の選択の両方を入れる案をユーザが選んだ）:

- **自動（D-91 の拡張）**: 承認前の CD の取り込みで、吸い出したリリースの front が 404 なら、サイドカーの
  `rip.metadata.release_group_id` があるとき**同じ回の中で**リリースグループの front
  （`release-group/{id}/front-500`。CAA がグループの代表に選んだ版の表の画像）を 1 度だけ試す
  （`import::cover::fetch_front`、`CoverArtClient::group_front`）。claim した 1 回の中の試行なので回数は増やさない。
  グループの 404 はリリースの 404 と同じく打ち止め、グループの上流の失敗はリリースの失敗と同じく次の走査で
  もう 1 回だけ。リリースに画像があればグループは引かない。どちらから取ったかはログ（`source`）にだけ残し、
  DB には持たない（`caa_picture` は置いた画像の値で、画面の「画像を外す」の判定にしか使わない。
  出どころを持つにはマイグレーションが要り、見返りが無い）。画面の文言は「吸い出したリリースかそのリリース
  グループから自動で取った」にする
- **手動**: Inbox の承認画面の画像の欄に「別のリリースから取る…」（`CaaReleasePicker`。曲ごとに画像が違う
  取り込みには出さない）。メタデータは取り込むリリースのまま、画像だけを置いて全曲の `picture` に入れる
  （従来の `applyPicture(…, 'all')`。配置で埋め込む）
  - 下書きの `release_group_id` があれば、同じグループの版を `GET /api/cd/release-group/{id}/releases`
    （MusicBrainz の browse `ws/2/release?release-group=…&inc=media+labels&limit=100`。既存の
    `MusicBrainzClient` の 1 req/s に乗る）で並べる。MusicBrainz のリリースは `cover-art-archive.front` を
    持つので、表の画像のある版だけをジャケット（`/api/cd/cover/{id}` の中継）付きで出し、無い版は件数だけ。
    並びは front のある版が先、同じなら日付の古い順。見分けに日付・国・形式（`CD + Blu-ray` 等）・レーベル・
    カタログ番号を出し、取り込むリリースに印を付ける。グループが 100 件を超えるときは先頭のぶんだけ
  - 「リリースグループの代表画像を取る」（`from-caa { release_group_id }`）
  - 別のグループに登録された版（BD 版が別のリリースグループになっている等）のために、MusicBrainz のリリースの
    URL か MBID の貼り付け。`from-caa { release_id }` はサーバで `parse_release_ref` を通すので URL も受ける
- `POST /api/artwork/from-caa` は `{ release_id }` か `{ release_group_id }` のどちらか片方（両方・無しは 400）

**理由**: 実機の嵐「Five」のリリースグループは CD + Blu-ray（画像あり）・CD + DVD-Video（画像なし）・配信
（画像なし）の 3 版で、DVD 付きを吸い出すと D-91 は画像なしで終わる。表の画像は版が違ってもほぼ同じで、
CAA もグループの代表画像を持っている。自動の補完で多くは埋まり、違う版の画像にしたいときだけ一覧から選ぶ。
これまでの回避策（別の版の画像をブラウザで保存してドロップする）は MusicBrainz を別に開く必要があった。

**却下**: 取り込むリリースをグループ内の画像のある版へ変える（メタデータ = カタログ番号・媒体構成・
`MUSICBRAINZ_ALBUMID` が実物の盤と食い違う）。手動を URL / MBID の貼り付けだけにする（変更は小さいが、別の版の
MBID を MusicBrainz で探して貼る手間が残る。ユーザの選択で一覧を付けた）。グループの版の一覧をキャッシュする
（開くのは件ごとに 1 回程度で、MusicBrainz の間隔制御に乗っていれば足りる）。

## D-94 CD の取り込みで品番（と JAN）を手で入れ、版の識別と照会に使う

**決定**（2026-09-26。P4-25。ユーザの要望: 「挿入した CD は通常盤なのに、MusicBrainz で一致したのは初回限定盤で、
そのまま取り込むとどの盤かという情報が落ちる。見た CD にはすべて品番（UPCJ-9085 のような）が付いていたので、
取り込み時に手で入れて保持すれば、後で版を直せる」。下の形をユーザが選んだ）:

- **CD 画面の ① に「品番」と「JAN」の入力欄を置く（どちらも任意）。** 値の補正ではなく照会と識別の入力で、
  D-67 追記の「CD 画面からは直せない」の例外にする（他の値は従来どおり Inbox の承認画面で直す）。
  入力は盤が替わったら消す
- **照会**: `POST /api/cd/lookup` に `catno?` を足す。リリース URL / MBID と同じく**段に関係なく常に引く**
  （DiscID で当たっても引く。通常盤と初回盤は収録が同じなら TOC も同じで、DiscID では版を区別できないため）。
  `ws/2/release?query=catno:"…"`（値は英数字・ハイフン・空白だけを通し、Lucene の記号を通さない。取得の上限は
  バーコードと同じ 3 件）。検索は大文字小文字・ハイフンの揺れを吸収するが部分一致もありうるので、リリースの
  label-info の品番のどれかが入力と**正規化して一致する**（大文字小文字・ハイフン・空白を無視）ものだけに
  経路 `catno` を付ける。JAN は既存の `barcode:` の検索に、盤から読めた MCN が無いときの入力として渡す
- **候補の並び**: 品番の入力があるときは、経路 `catno` を持つ候補を先頭にする（既定で選ばれる）。
  `matched_by` の強い順は `catno | discid | release | isrc | barcode | toc`。品番は実物から読んだ版の識別子で、
  DiscID が別の版にしか登録されていない盤では、品番で当たった版が手元の盤になる。収録の違う版
  （ボーナストラック付き等）は DiscID 自体が変わり、トラック数の合わない候補は従来どおり落ちる
- **下書きへの写し方**（写す範囲が「最小限」でも、入力した品番と JAN は書く。ユーザが実物から読んだ値のため）:
  - 選んだ候補の品番が入力と一致する → `CATALOGNUMBER` は候補の表記、`BARCODE` は候補の値
    （JAN の入力があり食い違えば入力値）
  - 食い違う（MusicBrainz に手元の版が無く、別の版で取り込む）→ 画面に注意を出し（例:「この候補の品番は
    UPCJ-9001（初回限定盤A）で、入力した品番と違います」）、版に固有の値を引き継がない:

    | タグ | 扱い |
    |---|---|
    | `CATALOGNUMBER` | 入力値 |
    | `BARCODE` | 引き継がない（JAN の入力があればそれ） |
    | アルバム名・アーティスト・トラック名・ISRC・`MUSICBRAINZ_TRACKID` | 引き継ぐ（DiscID / ISRC で中身が同じと確かめている） |
    | `LABEL`・`DATE` | 引き継ぐ（通常盤と初回盤で違うことはまず無く、日付も年はほぼ合う） |
    | `MUSICBRAINZ_ALBUMID`・`MUSICBRAINZ_RELEASETRACKID`・`MUSICBRAINZ_RELEASEGROUPID` | 引き継ぐ（近い版を指す値として） |
- **版がずれている印は DB に持たない。** `CATALOGNUMBER` が `MUSICBRAINZ_ALBUMID` のリリースの品番に無いことが
  印になり、タグと MusicBrainz から後で拾える（ファイルが正）。拾って版を直す画面は今回は作らない

**理由**: 実機で 7 枚取り込んだところ、DiscID で当たった候補が手元の盤と別の版のことがあった。DiscID は TOC から
出るので、収録が同じ版どうしは区別できない。品番は盤に必ず印刷されていて手入力の手間も小さく、MusicBrainz の
`catno:` 検索で版まで決まる（2026-09-26 に `catno:UPCJ-9001` → 「シンデレラガール」初回限定盤A の 1 件、
`catno:upcj9001` でも同じ 1 件を確認）。MusicBrainz に手元の版が無くても、品番がタグに残れば後で直せる。

D-72 の「カタログ番号や JAN は要らない」はこの範囲で覆す（値としてではなく、版の識別子として要る）。
D-72 の Discogs の却下はそのまま残す。品番の入力を入れたあと、MusicBrainz で当たらない盤がどれくらいあるかを
見て再検討する（TASKS の残課題）。

**却下**:
- 品番を「補助」に留め、DiscID / ISRC で当たった候補を常に優先する（収録が同じ版を区別できないので、
  品番で手元の版が見つかっても初回盤で取り込まれる）
- 食い違うとき `MUSICBRAINZ_ALBUMID` も外す（版に固有なので筋は通るが、ライブラリにあるかの判定（D-85）と
  ジャケットの自動取得（D-91 / D-93）が効かなくなる。品番との食い違いが後で版を直す手がかりとして残る）
- 食い違うとき候補の JAN を残す（品番が違えば JAN も別物で、実物と違う値がタグに残る）
- 版がずれている印を DB やサイドカーに持つ（タグと MusicBrainz から導ける。マイグレーションに見合わない）
- レーベルの公式サイトやタワーレコード / HMV を品番で引く（公開 API が無くスクレイピングになり、サイトごとに
  解析が要って改修で壊れる。規約上も自動取得は想定外）
