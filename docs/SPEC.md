...
- **CD**（P2、P4-20）: **ドライブに CD が入っている前提の画面**で、上がディスクのトラック表、下が
  MusicBrainz の候補。左カラム（ツリー）は出さない（§12.1）。
  ...
  - **トラック表**: TOC の音声トラックと 1:1 で、**照会の前から出る**（`GET /api/cd/status` の `tracks`。
    ...
    `進捗`。タイトルが空の行は `Track 01`… を**プレースホルダ**で見せ、候補を選ぶと実名に置き換わる。
    取り込みの直前に空のままなら `Track NN` を採用する（`finalizeDraft`）。ISRC は列にせず `#` の
    ツールチップ。見た目は一覧の表（`.track-table`）に寄せるが、コンポーネントは別
  - **アルバム欄**: アルバム / アルバムアーティスト / 日付 / category（`CategoryField`）/ ディスク番号・枚数を
    表の上に。レーベル・カタログ番号・JAN/UPC・トラックリスト貼り付け（P2-4、D-65）は「詳細」に畳む。
    TOC の貼り付け（CTDB 形式 / MusicBrainz 形式 / `cdrecord -toc` の出力）・各種 ID・用語の凡例も
    「詳細」（ドライブの無い環境とデバッグ用。P2-3、D-64）
  ...
  - **写す範囲**（D-72、P4-2）は既定で識別用の最小限（アルバム・アルバムアーティスト・日付・
    ディスク番号 / 枚数・`MUSICBRAINZ_ALBUMID`。`MUSICBRAINZ_DISCID` / `TRACKTOTAL` は吸い出し時に TOC から
    付く。トラック行は番号と長さだけの空行）、「全部写す」でレーベル・カタログ番号・JAN/UPC・
    `MUSICBRAINZ_RELEASEGROUPID`・各トラックのタイトル / アーティスト・MB id・ISRC も写る。範囲を
    切り替えると選択中の候補を写し直す（編集中の内容は消える）。候補ゼロ件でも空のフォームで完走できる
    （P2-4、D-21 / D-65）
  - **「確定」の段は無い**（P4-20）。表を直接編集し、「取り込む」で吸い出しが始まる（吸い出しは P2-5。
    それまではボタンを無効にしておく）。遷移は `lib/cdState.ts` の reducer: フォームは TOC が読めた時点で
    でき、消えるのは別のディスクに替わったとき（`set_disc` / `set_toc` / `reset`）だけ。**照会の開始と
...
- **Inbox**（P2-10、D-68）: 左に件（= `[paths].inbox` の音声ファイルのあるディレクトリ）の一覧
  ...
  右に選んだ件の補正フォーム: アルバムアーティスト / アルバム / 日付 / category（CD 画面と同じ
  `CategoryField`。P4-20 で `components/CdAlbumFields.tsx` へ移した）と、トラックごとの disc / # / タイトル / アーティスト（ファイル名・コーデック・長さは
