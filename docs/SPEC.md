...
- **CD**（P2、P4-20）: **ドライブに CD が入っている前提の画面**で、上がディスクのトラック表、下が
  ...
  - **アルバムの要約**（`CdAlbumSummary`）: 読み取り専用。アルバム名を見出しに、アルバムアーティスト /
    日付 / ディスク番号（複数枚組のときだけ）/ レーベル / カタログ番号 / JAN/UPC を 1 行に。値の無い
    項目は出さない。候補を選んでいなければ「（候補を選んでいない）」と、名前は Inbox で入れる旨を出す。
    **`category` は CD 画面に無い**（Inbox の承認時に選ぶ）。トラックリスト貼り付け（P2-4、D-65）も
    CD 画面からは外した（パーサは残してあり、Inbox へ移すのは別タスク）。TOC の貼り付け
