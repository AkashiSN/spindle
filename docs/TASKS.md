...
- ジョブの終端状態を DB に書けなかったとき（2026-09-20 の実機でディスク満杯により 88 本）、行が `running` の
  まま取り残され、次回起動のリカバリまで戻らない（設計どおり。`src/jobs/worker.rs::execute` のフォールバックも
  同じ理由で失敗する）。稼働中に回収する仕組み（`Jobs.running` に無い `running` 行を定期的に queued へ戻す、
  または終端書き込みをバックオフ付きで再試行）を入れるか。あわせて、docker のログも満杯で落ちるので
  WARN が残らない点をどう扱うか
