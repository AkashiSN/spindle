...
- [x] **P4-13** YouTube の導線を独立した画面に（D-70 追記、SPEC §12.6。2026-09-21。ジョブ一覧はサーバの
      `type=` フィルタを足さず web で絞る（完了 300 件まで載る）。再生リストの展開時に `SOURCE_URL` のある
      動画は投入しない = 実機で再生リストを貼ると新しいものだけ落ちる）。いまは右パネル「操作」タブの中の
