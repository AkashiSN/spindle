...
- **照合が通らない原因の調査（2026-09-24）**: 「Five」は新品で、CTDB にも同じ TOC で載っている（信頼度 30）。
  ...
  無視される（約 15 倍速のまま）。paranoia は jitter 補正を 60 秒あたり 1,300〜2,400 回行い、ときどき
  `drift` / `dropped` を起こしてトラック内で最大数百サンプルずれた PCM を出す → CRC がどのオフセットでも合わず、
  ...
  現状の `read_disc` は paranoia の `drift` / `dropped` を数えていない（rip.log の「再読」は repair / skip /
  read error だけ）
...
- **影響**: spindle はずれた吸い出しを「検証済み」にはせず mismatch として Inbox に置く（誤って正しいと言うことは
  ...
  いない。音は drift の箇所で数〜数百サンプル（最大 6 ms 程度）の欠け / 重複があり得て、ビット単位では正しくない
