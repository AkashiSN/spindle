...
3. **移行後の一度きりの手順**（webm 由来の `〜のお歌` アルバム。docs/TASKS.md P4-14 / P4-15）。
   ...
   3. YouTube 画面で 9 本の再生リストを購読に登録（アルバムアーティスト / アルバム = 各 `〜のお歌`、
      ...
      承認画面で「⚠ Library に同名」（P4-19）が出たら、**長さが一致するものは同じ音源の再アップロード**
      （= 二重取り込み）。その場合は承認せず、既存行の `SOURCE_URL` を再生リスト側の URL に付け替える
      （プロパティタブの Metadata）と、次の同期で「持っている」と判定されて落ちてこない。長さが違えば
      別テイク（Cover / Live ver.）なのでそのまま承認してよい。**Library のファイルを消す op は無い**ので、
