...
ユーザが foobar 側に貼り付ける。変換時の注意が 3 点ある。
...
拡張フィールドは foobar 側に存在しないため、変換できない旨をコメントで出力する。

**2. `ORDER BY` は分離する。** foobar の Autoplaylist はソートをクエリに書かず、
別欄のタイトルフォーマット文字列で指定する。エクスポートは
「クエリ」と「ソートパターン」の 2 本を出力する。

**3. `LIMIT` と `random` は変換不能。** 出力にコメントで明示する。

```
# spindle → foobar2000 Autoplaylist
# query:
%album artist% IS ヰ世界情緒 AND NOT %genre% IS _Unsorted
# sort pattern:
%date%  (降順にするには foobar 側で Reverse を指定)
# 変換できなかった指定:
#   LIMIT 100         … foobar の Autoplaylist に相当機能なし
#   %verification%    … spindle 固有フィールド
```
