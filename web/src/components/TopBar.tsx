// 上部バー（SPEC §12.1、D-58 追記）。ナビとプレイヤーを 1 本にまとめる:
// 左に画面切替（「一覧」がホーム。「ジョブ」はジョブ要約を件数バッジと失敗の赤点で示す）、
// 中央に ◀◀ ▶ ▶▶ ■ とシーク、その右に曲名（無ければ Not playing）、右に RG / 原本 / 音量と
// ☰（履歴 / 設定 / ログアウト。SSE の接続状態もここ）
      ...
      <nav className="views">
        {VIEWS.map(([v, label]) => (
          ...
          >
            ...
            {v === 'jobs' && summary ? (
              <>
                {active > 0 ? <span className="jobs-count">{formatCount(active)}</span> : null}
                {summary.failed > 0 ? <span className="jobs-failed" aria-label="失敗あり" /> : null}
              </>
            ) : null}
          </button>
        ))}
