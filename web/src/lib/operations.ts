...
export type HirescheckStartResponse = FlaccheckStartResponse
...
export function hirescheckStartedMessage(r: HirescheckStartResponse): string {
  const notes: string[] = []
  if (r.skipped > 0) notes.push(`対象外（非可逆・44.1/48 kHz かつ 16 bit・欠落） ${formatCount(r.skipped)}`)
  if (r.duplicates > 0) notes.push(`既に投入済み ${formatCount(r.duplicates)}`)
  return `偽ハイレゾ検出を投入した: ${formatCount(r.tracks)} 件${notes.length > 0 ? `（${notes.join(' / ')}）` : ''}`
}
